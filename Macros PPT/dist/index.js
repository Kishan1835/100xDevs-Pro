...
program
    ...
    .description("Migrate PowerPoint content from a source .pptx to a destination template .pptx, " +
    "preserving spatial layout while adopting the new theme.")
    .requiredOption("-s, --source <path>", "Source PowerPoint file (.pptx)")
    .requiredOption("-t, --template <path>", "Destination template PowerPoint file (.pptx)")
    .requiredOption("-o, --output <path>", "Output file path (.pptx)")
    .option("--slide-range <range>", 'Slide range to migrate (e.g., "1-5", "1,3,5", "2-")', undefined)
...
async function run(options) {
    ...
    if (!(await fs_extra_1.default.pathExists(options.source))) {
        throw new Error(`Source file not found: ${options.source}`);
    }
    if (!(await fs_extra_1.default.pathExists(options.template))) {
    ...
    const sourceBuffer = await fs_extra_1.default.readFile(options.source);
    const source = await (0, parser_1.parsePresentation)(sourceBuffer);
    console.log(`      Found ${source.slides.length} slides, ` +
    ...
    const templateBuffer = await fs_extra_1.default.readFile(options.template);
    const template = await (0, parser_1.parsePresentation)(templateBuffer);
    console.log(`      Found ${template.slides.length} slides, ` +
        `${template.slideLayouts.length} layouts, ` +
        `${template.slideMasters.length} masters`);
    // Step 3: Filter slides by range if specified
    let slidesToMigrate = source.slides;
    if (options.slideRange) {
        slidesToMigrate = filterSlidesByRange(source.slides, options.slideRange);
        console.log(`      Slide range "${options.slideRange}" selected: ${slidesToMigrate.length} slides`);
    }
    ...
    if (slidesToMigrate.length < beforeClosingFilter) {
        console.log("      Detected and skipped final closing 'Thank you' slide from source");
    }
    ...
    const mappings = (0, mapper_1.mapLayouts)(source, template, slidesToMigrate);
    for (const m of mappings) {
        console.log(`      Slide ${m.sourceSlide.index} -> "${m.destinationLayout.name}" (${m.matchType})`);
    }
    // Step 5: Transfer and build
    console.log("[4/5] Transferring content...");
    const { buffer, summary } = await (0, builder_1.buildMigratedPresentation)(source, template, mappings);
    // Step 6: Write output
    console.log(`[5/5] Writing output: ${options.output}`);
    await fs_extra_1.default.writeFile(options.output, buffer);
    // Print summary
...
*/
function filterSlidesByRange(slides, rangeStr) {
...
function removeClosingThankYouSlide(slides, totalSourceSlides) {
    if (slides.length === 0)
        return slides;
    const maybeLast = slides[slides.length - 1];
    if (maybeLast.index !== totalSourceSlides)
        return slides;
    if (!looksLikeThankYouSlide(maybeLast.xml))
        return slides;
    return slides.slice(0, -1);
}
function looksLikeThankYouSlide(slideXml) {
    const doc = domParser.parseFromString(slideXml, "text/xml");
    const textNodes = doc.getElementsByTagNameNS(types_1.NS.a, "t");
    let text = "";
