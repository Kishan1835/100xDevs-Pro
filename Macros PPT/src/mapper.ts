...
export function mapLayouts(
 ...
 sourceSlides: ParsedSlide[]
): LayoutMapping[] {
 const destLayouts = destination.slideLayouts;
 const basePreferredLayouts = destLayouts.filter(
   (l) => !isDividerLayoutName(l.name) && !isClosingLayoutName(l.name)
 );
 const candidateLayouts =
   basePreferredLayouts.length > 0 ? basePreferredLayouts : destLayouts;
 const sourceLayoutCache = new Map<string, SlideLayout | null>();
 ...
 for (const slide of sourceSlides) {
   ...
   const activeCandidates =
     perSlideCandidates.length > 0
       ? perSlideCandidates
       : nonFrontMatterLayouts.length > 0
         ? nonFrontMatterLayouts
         : candidateLayouts;

   let bestMatch: SlideLayout | null = null;
   let matchType: LayoutMapping["matchType"] = "blank-fallback";
   ...
   if (sourceLayoutName) {
     ...
     activeCandidates.find(
         (dl) => dl.name.toLowerCase().trim() === sourceLayoutName
       ) || null;
     if (bestMatch) {
   ...
   if (!bestMatch && sourceLayoutName) {
     bestMatch =
     activeCandidates.find((dl) =>
         dl.name.toLowerCase().trim().includes(sourceLayoutName) ||
         sourceLayoutName.includes(dl.name.toLowerCase().trim())
       ) || null;
     if (bestMatch) {
       matchType = "exact-name";
     }
   ...
   bestMatch =
    ...
    fallbackCursor++;
     ...
     if (!bestMatch) {
       console.error(
         `[ERROR] No layouts available in destination template for slide ${slide.index}`
       );
       // Use first layout anyway
       bestMatch = destLayouts[0];
     }
     console.warn(
       `[WARN] Slide ${slide.index}: No matching layout found, using "${bestMatch?.name || "unknown"}" as fallback`
     );
   }
...
*/
function findBlankLayout(layouts: SlideLayout[]): SlideLayout | null {
 // Try exact match first
 const blank = layouts.find(
   (l) => l.name.toLowerCase().trim() === "blank"
 );
 if (blank) return blank;

 // Try contains match
 const blankish = layouts.find((l) =>
   l.name.toLowerCase().includes("blank")
 );
 if (blankish) return blankish;

 // Find layout with fewest placeholders
 if (layouts.length > 0) {
   return layouts.reduce((min, l) =>
     l.placeholders.length < min.placeholders.length ? l : min
   );
 }
...
*/
function getSourceSlideLayout(
 source: ParsedPresentation,
 slide: ParsedSlide,
 cache: Map<string, SlideLayout | null>
): SlideLayout | null {
...
function isDividerLayoutName(name: string): boolean {
...
function isClosingLayoutName(name: string): boolean {
 const n = name.toLowerCase();
 return (
...
function slideHasImages(slide: ParsedSlide): boolean {
 for (const [, rel] of slide.relationships) {
   if (rel.type === REL_TYPES.image) return true;
 }
 return false;
}

function isImageFriendlyLayout(layout: SlideLayout): boolean {
...
function isWhiteCanvasLayoutName(name: string): boolean {
 const n = name.toLowerCase();
 return n.includes("white canvas") || n === "canvas";
}

function isFrontMatterLayoutName(name: string): boolean {
 const n = name.toLowerCase();
 return (
   n.includes("cover") ||
...
function pickRotatingFallbackLayout(
 layouts: SlideLayout[],
 cursor: number
): SlideLayout | null {
 if (layouts.length === 0) return null;

 const preferred = layouts.filter(
   (l) => !isWhiteCanvasLayoutName(l.name) && !isFrontMatterLayoutName(l.name)
 );
 const pool = preferred.length > 0 ? preferred : layouts;
 return pool[cursor % pool.length] || null;
}

function slideNeedsWhiteCanvas(slide: ParsedSlide): boolean {
 ...
 const pics = doc.getElementsByTagNameNS(NS.p, "pic");
 for (let i = 0; i < pics.length; i++) {
   const pic = pics[i];
   const ext = pic.getElementsByTagNameNS(NS.a, "ext");
 ...
 for (let i = 0; i < frames.length; i++) {
   ...
   const ext = frame.getElementsByTagNameNS(NS.a, "ext");
   if (ext.length === 0) continue;
   const cx = parseInt(ext[0].getAttribute("cx") || "0", 10);
   const cy = parseInt(ext[0].getAttribute("cy") || "0", 10);
   const hasChartRef =
     frame.getElementsByTagNameNS(
