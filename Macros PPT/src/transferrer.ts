...
*/
export async function transferSlide(
 sourceZip: JSZip,
 mapping: LayoutMapping,
 destSlideNumber: number,
 destLayoutRelTarget: string // relative path from new slide to layout, e.g., "../slideLayouts/slideLayout1.xml"
 ...
 const sourceSpTree = sourceDoc.getElementsByTagNameNS(NS.p, "spTree");
 ...
 const nvGrpSpPr = srcTree.getElementsByTagNameNS(NS.p, "nvGrpSpPr");
 const grpSpPr = srcTree.getElementsByTagNameNS(NS.p, "grpSpPr");
 ...
 if (nvGrpSpPr.length > 0) {
   newSpTree.appendChild(
     newSlideDoc.importNode(nvGrpSpPr[0], true)
   );
 }
 if (grpSpPr.length > 0) {
   newSpTree.appendChild(
     newSlideDoc.importNode(grpSpPr[0], true)
   );
 }

 // Build placeholder map for destination layout
 const destPlaceholderMap = new Map<string, typeof destinationLayout.placeholders[0]>();
 ...
 for (let i = 0; i < children.length; i++) {
   ...
   const cloned = newSlideDoc.importNode(element, true) as Element;

   // Handle placeholder shapes
   if (localName === "sp") {
     const handled = handlePlaceholderShape(
       cloned,
       destPlaceholderMap,
       newSlideDoc
     );
 ...
 const cSld = sourceDoc.getElementsByTagNameNS(NS.p, "cSld");
 if (cSld.length > 0) {
   const newCsld = newSlideDoc.getElementsByTagNameNS(NS.p, "cSld")[0];
   if (newCsld) {
 ...
 const transitions = sourceDoc.getElementsByTagNameNS(NS.p, "transition");
 if (transitions.length > 0) {
   const newSld = newSlideDoc.documentElement;
   newSld.appendChild(
     newSlideDoc.importNode(transitions[0], true)
   );
 }

 // Copy <p:timing> if present
 const timings = sourceDoc.getElementsByTagNameNS(NS.p, "timing");
 if (timings.length > 0) {
   const newSld = newSlideDoc.documentElement;
   newSld.appendChild(
     newSlideDoc.importNode(timings[0], true)
   );
 }
 ...
 const sourceSlideDir = sourceSlide.path.substring(
   0,
   sourceSlide.path.lastIndexOf("/")
 );

 // Collect existing rIds (will be populated as we create new ones)
 const destExistingRIds = new Set<string>();

 // Create the layout relationship first
 const layoutRId = generateUniqueRId(destExistingRIds);
 ...
 // Build the rels XML
 const slideRelsXml = buildRelsXml(newRelationships);
...
*/
function handlePlaceholderShape(
 sp: Element,
 destPlaceholderMap: Map<string, { type: string; idx: number; x: number; y: number; cx: number; cy: number }>,
 doc: Document
): Element {
 // Check if this sp has a <p:ph> child (inside <p:nvSpPr> -> <p:nvPr>)
 const nvPr = sp.getElementsByTagNameNS(NS.p, "nvPr");
 if (nvPr.length === 0) return sp;

 const phElements = nvPr[0].getElementsByTagNameNS(NS.p, "ph");
 if (phElements.length === 0) return sp;
...
*/
function findByType(
 map: Map<string, { type: string; idx: number; x: number; y: number; cx: number; cy: number }>,
 type: string
): { type: string; idx: number; x: number; y: number; cx: number; cy: number } | undefined {
 for (const [, ph] of map) {
...
function updateShapePosition(
 ...
 if (spPr.length === 0) return;

 let xfrm = spPr[0].getElementsByTagNameNS(NS.a, "xfrm");
 if (xfrm.length === 0) {
   // Create xfrm element
   const xfrmEl = sp.ownerDocument!.createElementNS(NS.a, "a:xfrm");
   const offEl = sp.ownerDocument!.createElementNS(NS.a, "a:off");
   offEl.setAttribute("x", x.toString());
   offEl.setAttribute("y", y.toString());
   const extEl = sp.ownerDocument!.createElementNS(NS.a, "a:ext");
 ...
 const off = xfrm[0].getElementsByTagNameNS(NS.a, "off");
 const ext = xfrm[0].getElementsByTagNameNS(NS.a, "ext");
...
*/
function ensureExplicitPosition(sp: Element): void {
 // Already in the XML from source — no action needed unless they're truly missing
 const spPr = sp.getElementsByTagNameNS(NS.p, "spPr");
 if (spPr.length === 0) return;

 const xfrm = spPr[0].getElementsByTagNameNS(NS.a, "xfrm");
 if (xfrm.length === 0) {
   // No transform at all — shape will be invisible. Add a default.
   console.warn(
     "[WARN] Shape converted from placeholder has no explicit position — may appear at (0,0)"
   );
 }
}
...
*/
function createBlankSlideDoc(): Document {
...
*/
function getOrCreateSpTree(doc: Document): Element {
 const spTree = doc.getElementsByTagNameNS(NS.p, "spTree");
 if (spTree.length > 0) return spTree[0];
 ...
 const tree = doc.createElementNS(NS.p, "p:spTree");
