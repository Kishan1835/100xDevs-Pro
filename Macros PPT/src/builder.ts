...
import { transferSlide } from "./transferrer";
import {
 copyFilesToDestZip,
 initCounters,
 resetCounters,
...
*/
export async function buildMigratedPresentation(
 source: ParsedPresentation,
 destination: ParsedPresentation,
 mappings: LayoutMapping[]
): Promise<{ buffer: Buffer; summary: MigrationSummary }> {
 ...
 for (let i = 0; i < mappings.length; i++) {
   ...
   const layoutRelTarget = getRelativePath(
     `ppt/slides/slide${destSlideNumber}.xml`,
     layoutPath
   );

   try {
     const result = await transferSlide(
       source.zip,
       mapping,
       destSlideNumber,
       layoutRelTarget
     ...
     const { copiedMedia, copiedCharts } = await copyFilesToDestZip(
       source.zip,
       destZip,
       result.mediaFiles,
       result.chartFiles,
     ...
     console.log(
       `  [OK] Slide ${mapping.sourceSlide.index} -> slide${destSlideNumber}.xml ` +
         `(layout: "${mapping.destinationLayout.name}", match: ${mapping.matchType})`
     );
   } catch (err) {
     console.error(
       `[ERROR] Failed to transfer slide ${mapping.sourceSlide.index}: ${err}`
     );
     summary.slidesSkipped++;
     summary.warnings.push(
       `Slide ${mapping.sourceSlide.index}: transfer failed — ${err}`
     );
   }
 }
...
*/
async function stripTemplateDividers(destZip: JSZip): Promise<void> {
 ...
 for (const xmlPath of candidatePaths) {
   ...
   const spTreeNodes = doc.getElementsByTagNameNS(NS.p, "spTree");
   if (spTreeNodes.length === 0) continue;
...
function shouldRemoveDividerShape(sp: Element): boolean {
 const cNvPr = sp.getElementsByTagNameNS(NS.p, "cNvPr");
 ...
 const prstGeom = sp.getElementsByTagNameNS(NS.a, "prstGeom");
 const hasLineGeom =
   prstGeom.length > 0 && (prstGeom[0].getAttribute("prst") || "") === "line";
 if (!hasLineGeom) {
   return false;
 }

 const textRuns = sp.getElementsByTagNameNS(NS.a, "t");
...
*/
function removeExistingSlides(
 destZip: JSZip,
 dest: ParsedPresentation
): void {
...
function updatePresentationXml(
 destZip: JSZip,
 dest: ParsedPresentation,
 slideEntries: { slideNumber: number; slidePath: string }[]
): void {
 // Update presentation.xml.rels
 const presRelsDoc = domParser.parseFromString(
   dest.presentationRelsXml,
   "text/xml"
 );
 ...
 const newSlideRIds: { rId: string; slideNumber: number }[] = [];
 let rIdCounter = 1;
 for (const rid of existingRIds) {
   const match = rid.match(/^rId(\d+)$/);
   if (match) {
     const num = parseInt(match[1], 10);
     if (num >= rIdCounter) rIdCounter = num + 1;
   }
 }
 ...
 for (const entry of slideEntries) {
   ...
   relEl.setAttribute("Type", REL_TYPES.slide);
 ...
 destZip.file(
   ...
   xmlSerializer.serializeToString(presRelsDoc)
 );
 ...
 let sldIdLst = presDoc.getElementsByTagNameNS(NS.p, "sldIdLst");
 let sldIdLstEl: Element;
 ...
 if (sldIdLst.length > 0) {
   ...
   sldIdLstEl = presDoc.createElementNS(NS.p, "p:sldIdLst");
   // Insert after <p:sldMasterIdLst> if it exists
   const masterIdLst = presDoc.getElementsByTagNameNS(NS.p, "sldMasterIdLst");
   if (masterIdLst.length > 0 && masterIdLst[0].nextSibling) {
     masterIdLst[0].parentNode!.insertBefore(
       sldIdLstEl,
       masterIdLst[0].nextSibling
     );
   } else {
 ...
 for (const entry of newSlideRIds) {
   ...
   sldId.setAttributeNS(NS.r, "r:id", entry.rId);
 ...
 destZip.file(
   ...
   xmlSerializer.serializeToString(presDoc)
 );
}
...
function updateContentTypes(
 ...
 const extensionsNeeded = new Set<string>();
 destZip.forEach((relativePath) => {
 ...
 destZip.file(
   "[Content_Types].xml",
   xmlSerializer.serializeToString(ctDoc)
 );
}
...
async function validateOutput(
 destZip: JSZip,
 slideEntries: { slideNumber: number; slidePath: string; relsPath: string }[]
): Promise<string[]> {
 ...
 if (ctFile) {
   ...
   for (const entry of slideEntries) {
     if (!ctXml.includes(`/${entry.slidePath}`)) {
       warnings.push(
         `[VALIDATION] Missing Content_Types entry for ${entry.slidePath}`
       );
     }
   }
 } else {
 ...
 if (presRelsFile) {
   ...
   for (const entry of slideEntries) {
     ...
     if (!presRelsXml.includes(target)) {
       warnings.push(
         `[VALIDATION] presentation.xml.rels missing reference to ${target}`
       );
     }
   }
 }
 ...
 for (const entry of slideEntries) {
   ...
   if (relsFile) {
     ...
     for (let i = 0; i < rels.length; i++) {
       ...
       if (rIds.has(rId)) {
         warnings.push(
           `[VALIDATION] Duplicate rId "${rId}" in ${entry.relsPath}`
         );
       }
       rIds.add(rId);
     }
   ...
   if (relsFile) {
     ...
     for (let i = 0; i < rels.length; i++) {
       ...
       if (resolvedPath && !destZip.file(resolvedPath)) {
         ...
         if (
           !resolvedPath.includes("slideLayout") &&
           !resolvedPath.includes("slideMaster")
         ) {
           warnings.push(
             `[VALIDATION] Referenced file missing in ZIP: ${resolvedPath} (from ${entry.relsPath})`
           );
         }
       }
     }
...
*/
function resolveFromSlideDir(slidePath: string, target: string): string {
